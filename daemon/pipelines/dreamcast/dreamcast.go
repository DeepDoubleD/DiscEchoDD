...
// PlanTranscode — transcode-half: DC skips transcode, runs compress
// (post-rip MD5 identify + chdman), move, notify.
...
// RunRip executes the drive-bound half: detect, identify, redumper cd
// (handles GD-ROM layout) → spoolDir/rip/rip.gdi + tracks, eject.
func (h *Handler) RunRip(ctx context.Context, drv *state.Drive, disc *state.Disc, prof *state.Profile, spoolDir string, sink pipelines.EventSink) (pipelines.RipResult, error) {
	...
	gdiPath := filepath.Join(ripDir, spoolName+".gdi")
...
// RunTranscode executes the compute-bound half: post-rip MD5
// identification (mutates disc in place if Redump matches), chdman
// compress, move (uses the mutated disc title/region), notify.
func (h *Handler) RunTranscode(ctx context.Context, result pipelines.RipResult, disc *state.Disc, prof *state.Profile, sink pipelines.EventSink) error {
	...
	gdiPath := filepath.Join(ripDir, spoolName+".gdi")
	// First .bin track (Redump MD5 reference): redumper zero-pads to two
	// digits starting at 01.
	track01Path := filepath.Join(ripDir, spoolName+"01.bin")
