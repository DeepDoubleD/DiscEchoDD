// Package netlink is a locally vendored, patched copy of
// github.com/pilebones/go-udev@v0.9.1's netlink package (GPL-3.0,
// see ../LICENSE), replaced in via the daemon's go.mod. The only
// change from upstream is in parseUdevEvent below -- see the comment
// there for why.
package netlink
...
func parseUdevEvent(raw []byte) (e *UEvent, err error) {
	...
	for _, envs := range fields[0 : len(fields)-1] {
		// PATCHED (upstream uses bytes.Split + len(env)!=2, which
		// aborts parsing the WHOLE event -- losing every property,
		// not just the offending one -- the instant any single env
		// VALUE contains a literal "=". Confirmed live: an
		// OmniDrive-flashed ASUS BW-16D1HT's uevents trip this
		// constantly (some vendor/model property value legitimately
		// contains "="), crashing the udev watcher and silently
		// dropping real disc-insert events during the reconnect
		// window. SplitN(..., 2) splits on the FIRST "=" only, which
		// is what every real udev env line ("KEY=value=with=equals")
		// actually means -- and matches how the kernel/udev itself
		// treats these lines.
