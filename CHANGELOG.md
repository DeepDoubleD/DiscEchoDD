...
### Changed
- Blu-ray (BDMV) rips now free the drive at eject instead of holding it through the HandBrake transcode. The pipeline is split into a drive-bound rip job (MakeMKV decrypt → eject) that lands its output in a new per-job spool under `${DISCECHO_DATA}/spool/<rip-job-id>/`, and a compute-bound transcode job that HandBrake-encodes the spooled MKV → moves to the library → fires the Apprise notification. On a multi-disc Blu-ray session this is the difference between "drive is busy for 85 minutes per disc" and "drive is free in ~25 minutes; transcodes drain in the background"; on a single disc it lets you reload the drive 30+ minutes sooner. The dashboard's drive card flips to `idle` the moment eject fires and the new transcode job surfaces alongside it in the queue. Apprise notification timing is unchanged from the user's perspective — it still fires after the move-step completes.
