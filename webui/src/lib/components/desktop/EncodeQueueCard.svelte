<script lang="ts">
  ...
  // Mirrors QueueTable but with the Drv column removed — transcode
  // jobs aren't bound to a drive (they live on the compute queue) so
  // the column would always render '—' for every row.
  export let jobs: Job[];
...
<div class="overflow-hidden rounded-2xl border border-border bg-surface-1">
  {#if jobs.length === 0}
    <div class="p-6 text-center text-[13px] text-text-3">No encodes in flight</div>
  {:else}
